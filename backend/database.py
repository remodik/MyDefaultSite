...
class File(Base):
    ...
    name: Mapped[str] = mapped_column(String(255))
    path: Mapped[str] = mapped_column(String(1024), default="", index=True)  # Full path like "folder1/folder2/file.txt"
    parent_path: Mapped[str] = mapped_column(String(1024), default="", index=True)  # Parent folder path
    is_folder: Mapped[bool] = mapped_column(Boolean, default=False, index=True)  # True if this is a folder
...
async def init_models() -> None:
    async with engine.begin() as conn:
        ...
        try:
            # Check if path column exists, if not add folder support columns
            check_column_sql = """
            SELECT column_name
            ...
            # SQLite doesn't have information_schema, so this will fail
            # That's okay, SQLite will use create_all which recreates tables
