        ...
        <div class="file-tree">
            ${
              tree.length > 0
                ? tree
                    .map((item) =>
                      renderTreeItem(item, onSelect, null, projectId),
                    )
                    .join("")
...
function attachTreeEventListeners(container, files, onSelect, projectId) {
  container
    .querySelectorAll(".file-tree-toggle:not(.empty)")
    .forEach((toggle) => {
  ...
  container.querySelectorAll(".file-tree-item-content").forEach((content) => {
    content.addEventListener("click", (e) => {
      if (
        e.target.closest(".file-tree-action-btn") ||
        e.target.closest(".file-tree-toggle")
      ) {
        return;
      }
    ...
    menuItems.push(
      {
        ...
        label: "New File",
        action: () =>
          createNewFile(item, projectId, files, onSelect, containerId),
      },
      {
        icon: "fa-folder-plus",
        label: "New Folder",
        action: () =>
          createNewFolder(item, projectId, files, onSelect, containerId),
      },
      { divider: true },
    );
  }
  ...
  menuItems.push(
    {
      icon: "fa-edit",
      label: "Rename",
      action: () => renameItem(item, projectId, files, onSelect, containerId),
    },
    {
      icon: "fa-trash",
      label: "Delete",
      action: () => deleteItem(item, projectId, files, onSelect, containerId),
      danger: true,
    },
  );
    ...
    if (createBtn) {
      createBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          const updatedProject = await import("../api.js").then((m) =>
            m.projectsApi.getById(projectId),
          );
          renderFileTree(
    ...
    if (item.is_folder) {
      content.addEventListener("dragover", (e) => {
        ...
        if (
          ...
          !item.path.startsWith(draggedItem.path)
        ) {
